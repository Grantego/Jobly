...
class User {
  ...
  static async get(username) {
    ...
    const applications = await db.query(`SELECT job_id FROM applications WHERE username = $1`,[username] )

    user.jobs = applications.rows.map(j => j.job_id)
  ...
  /** Allows user to apply for specific job
   * 
   * also checks that username and jobId are valid
   * 
