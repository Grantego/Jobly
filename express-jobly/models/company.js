...
class Company {
  ...
  static async findAll(q = {}) {
    ...
    let {name, minEmployees, maxEmployees} = q
    ...
    if (minEmployees !== undefined) {
      ...
      queries.push(`num_employees >= $${values.length}`)
    }
    ...
    if(name !== undefined) {
      values.push(name)
      queries.push(`name ILIKE $${values.length}`)
    }
    ...
    const companiesRes = await db.query(statement, values);
  ...
  static async get(handle) {
    ...
    const jobs = await db.query(`
      SELECT id, title, salary, equity::float
      FROM jobs
