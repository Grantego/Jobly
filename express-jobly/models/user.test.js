...
describe("apply", function() {
  test("works with valid username and jobId", async function() {
    const res = await User.apply('u1', 1)
    expect(res).toEqual({applied: 1})

    const check = await db.query(`SELECT * FROM applications`)
    expect(check.rows.length).toEqual(1)
  })

  test("not found if invalid username", async function() {
    try {
      await User.apply('ufdsa', 1)
    } catch(e) {
      expect(e instanceof NotFoundError).toBeTruthy()
    }
  })

  test("not found if invalid jobId", async function() {
    try {
